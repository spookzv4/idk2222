...
local MainFolder, ConfigFolder = "LavaBounce", "LavaBounce/configs"
local ConfigSetting = {ToggleButton = {MiniToggle = {}, Sliders = {}, Dropdown = {}}}
local MainFile = nil
local Library = {}
if not shared.LavaBounce then
	shared.LavaBounce = {
		Uninjected = false,
		Visual = {
			Hud = true,
...
if isfolder(MainFolder) and isfolder(ConfigFolder) then
	...
	spawn(function()
		RunService.RenderStepped:Connect(function()
			if AutoSave then
				if shared.LavaBounce.Uninjected then
					AutoSave = false
				end
...
function Library:CreateMain()
		...
		RunService.RenderStepped:Connect(function()
			if ScreenGui then
				if shared.LavaBounce.Uninjected then
					task.wait(1.2)
					ScreenGui:Destroy()
					shared.LavaBounce.Uninjected = false
				end
			end
		end)
	end)
	...
	spawn(function()
		RunService.RenderStepped:Connect(function()	
			if HudFrame then
				if shared.LavaBounce.Visual.Hud then
	...
	spawn(function()
		RunService.RenderStepped:Connect(function()	
			if HudFrame and LibraryTitle then
				if shared.LavaBounce.Visual.Watermark then
	...
	spawn(function()
		RunService.RenderStepped:Connect(function()	
			if HudFrame and ArrayFrame then
				if shared.LavaBounce.Visual.Arraylist then
